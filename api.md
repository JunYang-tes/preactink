...
```jsx
import {render, Box, Text} from 'preactink';
...
```jsx
import {render, Text, Newline} from 'preactink';
...
```jsx
import {render, Box, Text, Spacer} from 'preactink';
...
import {render, Static, Box, Text} from 'preactink';
...
```jsx
import {render, Transform} from 'preactink';
...
A scrollable container for managing large content that exceeds the visible area. It supports programmatic scrolling and visual indicators like scrollbars and percentage progress.

```jsx
import {render, ScrollView, Text} from 'preactink';

const Example = () => (
	<ScrollView height={10} showScrollbar showPercentage>
...
The height of the visible area in lines. If not provided, it will be calculated based on the container's dimensions.
...
Whether to display a scrollbar on the right side.
...
Whether to display the scroll progress percentage at the top-right corner.

##### style

Type: `object`

Styles to apply to the scroll container. Supports all [`<Box>`](#box) style properties.
...
import {render, ScrollView, Text, Box} from 'preactink';
...
const Example = () => {
	...
	return (
		<Box flexDirection="column">
			...
			<Box>
				<Text onPress={() => scrollViewRef.current.scrollUp()}>↑</Text>
				<Text onPress={() => scrollViewRef.current.scrollDown()}>↓</Text>
			</Box>
		</Box>
	);
};
...
```jsx
import {useInput} from 'preactink';
...
```js
import {useStdin} from 'preactink';
...
```js
import {useStdout} from 'preactink';
...
```js
import {useStderr} from 'preactink';
...
```jsx
import {render, useFocus, Text} from 'preactink';
...
```js
import {useFocusManager} from 'preactink';
...
```js
import {useFocusManager, useInput} from 'preactink';
...
Use alternative screen buffer,this mean the app will takes up the whole screen.
...
```jsx
import {render, measureElement, Box, Text} from 'preactink';
...
- [Scrollable](examples/scrollview.tsx) - Use `<ScrollView>` to render scrollable content.
